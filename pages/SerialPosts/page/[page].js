...
export async function getStaticProps(context) {
  ...
  const res = await fetch(`http://localhost:27017/serial?page=${con}`)
  const json = await res.json()

  const resSlider = await fetch('http://localhost:27017/allPost/')
...
export async function getStaticPaths (){
  const resSlider = await fetch('http://localhost:27017/serial/')
  const JsonSlider = await resSlider.json()
