...
export default function Home (props) {
    ...
    const filterIran = props.serial.filter(se => {
   ...
   const display = filterGenres.reverse().slice(pageVisited, pageVisited + moviesPerPage).map(movi => {
        ...
        <a style={{textDecoration:"none"}} href={`/movie/${movi._id}`} ><h3>با زیرنویس فارسی {movi.title} دانلود سریال </h3></a>
        ...
        <p>داستان سریال : <p  style={{opacity:"0.7",position:"relative",top:"-18px"}}>{movi.story}</p></p>
